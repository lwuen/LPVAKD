...
@torch.no_grad()
def test_production(model, predictor, val_data, inference_data, test_edge_bundle, negative_samples, evaluator,
                    batch_size, encoder_name, dataset):
    model.eval()
    predictor.eval()
    if encoder_name == 'mlp':
        h = model(val_data.x)
    ...
    results['AUC'] = (roc_auc_score(valid_result.cpu().numpy(), valid_pred.cpu().numpy()),
                      roc_auc_score(test_result.cpu().numpy(), test_pred.cpu().numpy()),
                      roc_auc_score(old_old_result.cpu().numpy(), old_old_pred.cpu().numpy()),
                      roc_auc_score(old_new_result.cpu().numpy(), old_new_pred.cpu().numpy()),
                      roc_auc_score(new_new_result.cpu().numpy(), new_new_pred.cpu().numpy()))
...
def main():
    ...
    parser.add_argument('--datasets', type=str, default='citeseer')
    ...
    os.makedirs("../saved-nodes", exist_ok=True)
    ...
    if args.transductive == "transductive":
        ...
        if exists("../data/" + args.datasets + "_production.pkl"):
            training_data, val_data, inference_data, _, test_edge_bundle, negative_samples, _= torch.load("../data/" + args.datasets + "_production.pkl")
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            if args.transductive == "transductive":
                loss = train(model, predictor, data, split_edge, optimizer, args.batch_size, args.encoder,
                             args.datasets, args.transductive)
                results, h = test_transductive(model, predictor, data, split_edge, evaluator, args.batch_size, args.encoder, args.datasets, args)
            else:
                loss = train(model, predictor, training_data, None, optimizer, args.batch_size, args.encoder,
                             args.datasets, args.transductive)
                results, h = test_production(model, predictor, val_data, inference_data, test_edge_bundle,
                                             negative_samples, evaluator, args.batch_size, args.encoder, args.datasets)
